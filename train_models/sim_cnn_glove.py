...
class CNN_Model(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(nn.Conv1d(200, 512, 3, 1, 1), nn.MaxPool1d(3, 1, 1), nn.ReLU())
    ...
    def forward(self, text):
        embedded = text.permute(0, 2, 1)  # [batch_size, embedding_dim, seq_len]

        out = self.conv1(embedded).nan_to_num()
        out = self.conv2(out).nan_to_num()
        out = self.conv3(out).nan_to_num()
        out = self.fcn(out.permute(0, 2, 1))
        output = self.dropout(out)
        return output.nan_to_num()

class Siamese_CNN_glove_Model(nn.Module):
    def __init__(self) -> None:
        super(Siamese_CNN_glove_Model, self).__init__()
        self.padding_idx = 0
        self.CNN = CNN_Model()
